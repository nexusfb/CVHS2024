...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    x_coords = np.linspace(border, w - border, nPointsX)
    y_coords = np.linspace(border, h - border, nPointsY)
...
def bow_histogram(vFeatures, vCenters):
    ...
    for i in range(M):
        # Calculate the Euclidean distance between the descriptor and all cluster centers
...
def create_bow_histograms(nameDir, vCenters):
    ...
    for i in tqdm(range(nImgs)):
        ...
        # Compute the BoW histogram for the image
